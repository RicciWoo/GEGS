...
#include <math.h>
/*
* This is /usr/people/omega/progs/ctcreate/dicom/RCS/ReadCT_DICOM.c,v
* Revision 1.1 last edited  2001-02-06 16:53:22-05
...
*
*   Code provided by Nick Rynaert, University of Gent, Belgium, May '04  *
*   Modified by Blake Walters, June '04                                  * 
*/
  ...
  if(len>=hwm_buffle)
    {
      ...
      if(buffle==NULL)
        {
	  if( (buffle = (char*)malloc(hwm_buffle)) == NULL )
	    {
	...
	{
	  if( (buffle = (char*)realloc(buffle,hwm_buffle)) == NULL )
	    {
	      fputs(" Cannot alloc a sufficient amount of memory.\n",stderr);
	      exit(1);
	      return -1;
...
*/
void readct_dicom (int ct_arraysize[3],
		           short *ctdata, float ct_offset[3],
	               float ct_voxelsize[3], int *ct_errorcode)
{
  int nrows,ncolumns,jump=0,prev;
  //int itel;
  int ij=0, iresol_half=0, i=0, j=0, num_slices, slice, count, num_overlap=0;
  //int slice_thick_def=0;
  //int slice_loc_def=0;
  int ct_negative=0, zerocount;
  //int offset=0;
  long jtel=-1, mtel;
  ...
  int ct_orient[10];
  FILE *file_ptr,*fi;
  int next_slice[MAX_SLICES],first_slice;
  //int nxt;
  Elemh h;
  char 
 ...
 /* first read all files and sort them */ 
  ...
  string dicom_dir = string(CTFileName);  //add by Wu 20170110
  int ipos = dicom_dir.rfind('\\');  //add by Wu 20170110
  dicom_dir.erase(ipos+1);  //add by Wu 20170110
  //string slicename_str = string(dicom_dir);  //for relative path in dicom file list
  string slicename_str;  //for relative path in dicom file list, changed by Wu 20170110
  for (slice = 0; slice < num_slices; slice++)
  {
    count++;
	//slicename_str = string(dicom_dir) + string(slicename[count]);  //for relative path in dicom file list
	slicename_str = dicom_dir + string(slicename[count]);  //for relative path in dicom file list, changed by Wu 20170110
    if( (fi=fopen(slicename_str.c_str(),"rb")) == NULL )  //for relative path in dicom file list, changed by Wu 20170110
	//if( (fi=fopen(slicename[count],"rb")) == NULL )  //for absolute path in dicom file list, changed by Wu 20170110
    {
  ...
  { 	  
	  ...
	  slicename_str = dicom_dir + string(slicename[slice]);  //for relative path in dicom file list, by Wu 20170110
	  if( (fi=fopen(slicename_str.c_str(),"rb")) == NULL )  //for relative path in dicom file list, by Wu 20170110
	...
	case image_orientation_tag:
	   ...
	   *ct_errorcode=1;
	   ...
	   //printf(" Image orientation: %i %i %i\n",ct_orient[0],ct_orient[4],ct_orient[8]);
	  }
	  break;
	...
	case image_position_tag:
	   ...
	   *ct_errorcode=1;
	   ...
	   //printf("\n Image position: %f %f %f\n",ct_offset[0],ct_offset[1], ct_offset[2]);
          }
	  break;
       ...
       switch( h.tag )
	{
        ...
        /* this is only here to determine whether the difference between
          ...
          else
          {
           ...
           //printf("\n Image position: %f %f %f\n",ct_offset[0],ct_offset[1],z_final-ct_voxelsize[2]/2.);
...
*/
 ...
 //printf("X range : %f - %f cm\n",ct_offset[0],ct_offset[0]+ct_arraysize[0]*ct_voxelsize[0]);
 //printf("Y range : %f - %f cm\n",ct_offset[1],ct_offset[1]+ct_arraysize[1]*ct_voxelsize[1]);
